...
when (io.csr_regWrite === 1.B) {        // When 'csr_regWrite' is asserted, 
    vtypeReg := vtype_encod             //update 'vtypeReg' with 'vtype_encod, only when there is vsetvli instruction.
    vlReg := io.vl_writeback            //update 'vlReg' with 'io.vl_writeback', only when there is vsetvli instruction.
}
