  ...
  switch(opcode) {
    ...
    is("b1010111".U) { // vector operations
      ...
      when(configtype === "b00".U || configtype === "b01".U) { // vsetvli
        ...
        io.Ex_sel := 3.U
        io.nextPCsel := "b00".U
      }.elsewhen(func3 === "000".U) { // vector-vector
        io.RegWrite := 1.B
        io.MemWrite := 0.B
        io.Branch := 0.B
        io.MemRead := 0.B
      ...
      }.elsewhen(func3 === "100".U) { // vector-scalar
        io.RegWrite := 1.B
        io.MemWrite := 0.B
        io.Branch := 0.B
        io.MemRead := 0.B
        ...
        io.nextPCsel := "b00".U
        io.V_inst := 1.U
      }.elsewhen(func3 === "011".U) { // vector-immediate
        io.RegWrite := 1.B
        io.MemWrite := 0.B
        io.Branch := 0.B
        io.MemRead := 0.B
    ...
    is("b0000111".U) { // load-vector
      when(
        io.Instruction(27, 26) === "b00".U && io
          .Instruction(24, 20) === "b00000".U
      ) { // (mop =00) unit stride// (lumop =000000) unit stride load
        io.RegWrite := 1.B
        io.MemWrite := 0.B
        io.Branch := 0.B
        io.MemRead := 1.B
