...
class Vregfile ( numElem: Int, elemWidth: Int) extends Module {
  val io = IO (new Bundle {
    ...
    val vd_data = Input(Vec(numElem, UInt(elemWidth.W)))
    val reg_write = Input(Bool())
    val vs1_data = Output(Vec(numElem, UInt(elemWidth.W)))
    val vs2_data = Output(Vec(numElem, UInt(elemWidth.W)))
  })
